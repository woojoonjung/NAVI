...
class Trainer:
    def __init__(self,
        ...
        data_weight=None,
        device=None,
        save_ckpt_start_epoch=None,
        save_ckpt_every=None,
        **kwargs,
        ):
        self.flag = flag
        self.save_ckpt_start_epoch = save_ckpt_start_epoch
    ...
    def _should_save_periodic_checkpoint(self, epoch):
        """1-based epoch index: save at start_epoch, start+every, ..."""
        if self.save_ckpt_start_epoch is None or self.save_ckpt_every is None:
            return False
        if self.save_ckpt_start_epoch <= 0 or self.save_ckpt_every <= 0:
            return False
        one_based = epoch + 1
        if one_based < self.save_ckpt_start_epoch:
            return False
        return (one_based - self.save_ckpt_start_epoch) % self.save_ckpt_every == 0
    ...
    def train(self, eval_data=None):
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            if self.test_set_list is not None:
                ...
                if self._should_save_periodic_checkpoint(epoch):
                    ckpt_dir = os.path.join(self.output_dir, 'epoch_{}'.format(epoch + 1))
                    os.makedirs(ckpt_dir, exist_ok=True)
                    self.save_model(ckpt_dir)
                    logger.info('saved periodic checkpoint to {}'.format(ckpt_dir))
