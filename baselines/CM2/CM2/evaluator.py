...
def auc_fn(y, p, num_class=2):
    if num_class > 2:
        # Without `labels`, sklearn infers classes from y only and expects p.shape[1] == len(unique(y)),
        # which breaks when a fold omits some classes. Fix label space to full num_class.
        return roc_auc_score(
            y, p, multi_class='ovo', labels=np.arange(num_class)
        )
    else:
