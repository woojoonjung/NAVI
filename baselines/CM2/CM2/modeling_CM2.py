...
# Artifacts live next to this file (baselines/CM2/CM2/), not cwd-relative ./CM2/
_CM2_PKG_DIR = os.path.dirname(os.path.abspath(__file__))


def _cm2_asset(*parts: str) -> str:
    return os.path.join(_CM2_PKG_DIR, *parts)
...
class CM2WordEmbedding(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        word2vec_weight = torch.load(_cm2_asset('bert_emb.pt'))
        ...
        weight_emb = torch.load(_cm2_asset('bert_layernorm_weight.pt'))
        bias_emb = torch.load(_cm2_asset('bert_layernorm_bias.pt'))
...
class CM2FeatureExtractor:
    def __init__(self,
        ...
        **kwargs,
        ...
        _tok_dir = _cm2_asset('tokenizer')
        if os.path.exists(_tok_dir):
            self.tokenizer = BertTokenizerFast.from_pretrained(_tok_dir)
        else:
            self.tokenizer = BertTokenizerFast.from_pretrained('bert-base-uncased')
            os.makedirs(_tok_dir, exist_ok=True)
            self.tokenizer.save_pretrained(_tok_dir)
...
class CM2TransformerLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # PyTorch >=2.1 nn.TransformerEncoder passes is_causal into each layer; CM2 is bidirectional — ignore.
        _ = (is_causal, kwargs)
        x = src
        if self.use_layer_norm:
            if self.norm_first:
...
class CM2ForMask(CM2Model):
    ...
    def forward(self, x, y=None, table_flag=0, masked_indices_override=None, return_aux=False):
        if isinstance(x, dict):
            inputs = x
        elif isinstance(x, pd.DataFrame):
            inputs = self.input_encoder.feature_extractor(x, table_flag=table_flag)
        ...
        if masked_indices_override is not None:
            masked_indices = masked_indices_override.to(self.device)
        ...
        if num_encoder_output.nelement() != 0:
            ...
            if return_aux:
                aux['num_proj'] = num_encoder_output
                aux['num_mask'] = masked_indices[:, :num_cnt]
                aux['x_num'] = other_info['x_num']
        ...
        if cat_encoder_output.nelement() != 0:
            ...
            if return_aux:
                aux['cat_proj'] = cat_encoder_output
                aux['cat_mask'] = masked_indices[:, num_cnt:]
                aux['cat_bert_emb'] = other_info['cat_bert_emb']
       
        loss = 0.5*num_loss + cat_loss

        if return_aux:
            aux['num_loss'] = num_loss
            aux['cat_loss'] = cat_loss
