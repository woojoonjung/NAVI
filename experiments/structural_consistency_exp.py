...
def load_model(model_name, domain, config_suffix=""):
    ...
    if config_suffix:
        ...
        domain_cap = domain_mapping.get(domain, domain.capitalize())
        
        # Add ablation variants
        ablation_variants = {
            'woED': f'./models/woED_Quarter_{domain_cap}_HVB_{config_suffix}/epoch_2',
            'woSI': f'./models/woSI_Quarter_{domain_cap}_HVB_{config_suffix}/epoch_2',
            'woSMLM': f'./models/woSMLM_Quarter_{domain_cap}_HVB_{config_suffix}/epoch_2',
            'full': f'./models/full_Quarter_{domain_cap}_HVB_{config_suffix}/epoch_2'
        }
    ...
    elif model_name in ['atlas', 'woED', 'woSI', 'woSMLM', 'full']:
        if model_name != 'atlas':
            model = AtlasForMaskedLM(model_path, ablation_mode=model_name)
        else:
...
def _get_dataset_for_row(row_json, model_name):
    if model_name in ['atlas', 'woED', 'woSI', 'woSMLM', 'full']:
        return AtlasDataset([row_json])
...
def get_primary_row_embedding(row_json: dict, model, model_name: str):
    ...
    (Segment-based for Atlas, mean-pooled tokens for baselines).
    """
...
def get_segment_embeddings(row_json: dict, model: AtlasForMaskedLM):
    """Extracts segment embeddings for an Atlas model for a single row."""
...
def run_structural_consistency_experiment(domain: str, model_name: str, artifacts_dir: Path, data_dir: Path, 
    ...
    for row_json in tqdm(sampled_rows, desc=f"Processing rows for {model_name}"):
        ...
        S_orig = get_segment_embeddings(row_json, model) if model_name in ['atlas', 'woED', 'woSI', 'woSMLM', 'full'] else None
        ...
        for k in range(n_permutations):
            ...
            results.append({
                ...
                'permutation_k': k, 'embedding_type': 'mean_pooled_segment' if model_name in ['atlas', 'woED', 'woSI', 'woSMLM', 'full'] else 'mean_pooled_token',
...
def main():
    ...
    parser.add_argument('--ablation_models', type=str, nargs='+', default=['woED', 'woSI', 'woSMLM', 'full'], help="Ablation models to run.")
