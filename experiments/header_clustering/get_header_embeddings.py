...
try:
    ...
    _checkpoint_epoch = getattr(_app_config, "CHECKPOINT_EPOCH", 4)
except ImportError:
    _checkpoint_epoch = 4
...
def get_model_path(model_name: str, domain: str, ablation_mode: bool = False) -> str:
    ...
    if ablation_mode:
        ...
        return f'./models/{model_name}/epoch_{ep}'
    else:
        # For regular models, use the simple mapping - fixed to use correct model directory names
        model_path_map = {
            'bert': f'./models/bert_{domain}/epoch_{ep}',
            'tapas': f'./models/tapas_{domain}/epoch_{ep}',
            'haetae': f'./models/haetae_{domain}/epoch_{ep}',
            'navi': f'./models/navi_{domain}/epoch_{ep}'
        }
...
def get_base_model_name(model_name: str, ablation_mode: bool = False) -> str:
    ...
    if ablation_mode:
        ...
        if 'navi_' in model_name and ('woESA' in model_name or 'woSSI' in model_name or 'woMSM' in model_name or 'woGHA' in model_name or 'woGHC' in model_name):
            return 'navi'  # All ablation models appear to be NAVI variants
        # Check for old naming convention for backward compatibility
        elif 'woESA' in model_name or 'woSSI' in model_name or 'woMSM' in model_name or 'woGHA' in model_name or 'woGHC' in model_name or 'full_' in model_name:
            return 'navi'  # All ablation models appear to be NAVI variants
        else:
...
def load_model(model_name: str, domain: str, ablation_mode: bool = False):
    ...
    if base_model_name == 'navi':
        epoch_suffix = f'/epoch_{_checkpoint_epoch}'
        base_path = model_path.rsplit(epoch_suffix, 1)[0] if epoch_suffix in model_path else model_path
        epoch_path = find_epoch_path(base_path, epoch=_checkpoint_epoch)
        if not epoch_path:
            logging.error(f"NAVI model epoch_{_checkpoint_epoch} not found for {model_name} in domain {domain} at base path: {base_path}")
            return None
    ...
    if base_model_name == 'bert':
        ...
        haetae_tokenizer = BertTokenizer.from_pretrained(_bert_name, **_bert_kw)
        model = HAETAE(bconfig, haetae_tokenizer, model_path)
    elif base_model_name == 'navi':
        # Model ablation_mode only for woGHA/woGHC; woSSI/woMSM/woESA are dataset/training-only
        model_ablation = 'full'
        if ablation_mode and model_name:
            for ab in ('woGHA', 'woGHC'):
                if ab in model_name:
                    model_ablation = ab
                    break
