...
def load_model(model_name, domain, config_suffix=""):
    ...
    if model_name in ['navi', 'woSSI', 'woMSM', 'woESA', 'woGHA', 'woGHC', 'full']:
        ...
        if not epoch_path:
            logging.error(f"NAVI model epoch_{_checkpoint_epoch} not found for {model_name} in domain {domain} at base path: {model_path}")
            return None
...
def main():
    ...
    parser.add_argument('--output_dir', type=Path, default='experiments/robustness_analysis/robustness_results', help="Output directory for results.")
    parser.add_argument('--domains', type=str, nargs='+', default=['cleaned/Movie', 'cleaned/Product'], help="List of domains to process.")
    parser.add_argument('--models', type=str, nargs='+', 
                        default=['bert', 'tapas', 'haetae', 'navi', 'woSSI', 'woMSM', 'woESA', 'woGHA', 'woGHC'],
                        help="Models to run. Options: bert, tapas, haetae, navi, woSSI, woMSM, woESA, woGHA, woGHC")
