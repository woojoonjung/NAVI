...
from experiments.robustness_analysis.utils import (
    load_data,
...
def create_variant_datasets(domain: str, synonym_map_path: str, random_seed: int = 42):
    ...
    # Define input paths
    base_dir = Path(DATA_ROOT) / "cleaned" / domain / "test"
    cls_input_path = base_dir / f"WDC_{domain_lower}_for_cls.jsonl"
    mp_input_path = base_dir / f"WDC_{domain_lower}_for_mp.jsonl"
    
    # Check if input files exist
    if not cls_input_path.exists():
        ...
        cls_data = load_data(str(cls_input_path))
        print(f"   Loaded {len(cls_data)} rows")
    
    if not mp_input_path.exists():
        ...
        mp_data = load_data(str(mp_input_path))
